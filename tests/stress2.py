...
def read_one_http_response(
    sock, read_timeout, max_bytes=1 << 20, leftover=b"", request_method=None
):
    """
    Returns (raw_response_bytes, unconsumed_leftover_bytes, truncated).
    `truncated=True` means the connection closed before Content-Length (or
    the chunked terminator) was actually satisfied -- i.e. the response
    LOOKS well-formed (valid status line, valid headers) but the body is
    shorter than promised. Callers must check this explicitly; a truncated
    response is not "ok" just because its status line parses fine.
    """
    ...
    while b"\r\n\r\n" not in buf:
        ...
        if not chunk:
            return buf, b"", True  # closed before headers even completed
        buf += chunk
    ...
    if is_chunked:
        ...
        while True:
            ...
            if len(data) >= max_bytes:
                truncated = False  # hit our own size cap, not a real truncation
                break
        return head + b"\r\n\r\n" + data, b"", truncated

    # Neither Content-Length nor chunked: body is legitimately close-terminated
    # (common for some minimal error responses). Reading until close/timeout
    # IS the correct framing here, not a truncation.
    data = rest
    try:
        while True:
            chunk = sock.recv(4096)
...
def print_report(stats: Stats, elapsed: float):
    ...
    for cat in sorted(by_cat):
        ...
        if d["statuses"]:
            status_str = ", ".join(
                f"{code}: {c}" for code, c in sorted(d["statuses"].items())
            )
            print(f"  status codes:  {status_str}")
        if d["errors"]:
            err_str = ", ".join(
                f"{label}: {c}" for label, c in sorted(d["errors"].items())
            )
            print(f"  ERRORS:        {err_str}")
        if lat_sorted:
            print(
                f"  latency (ms):  min={lat_sorted[0] * 1000:.1f}  "
    ...
    econnreset = sum(
        1 for _, ok, _, err, _ in records if not ok and err == "ECONNRESET"
    )
    print(f"\n{'-' * 70}")
...
def do_single_request(method, path, headers, body, keep_alive, sock=None, leftover=b""):
    """
    ...
    `leftover` must be whatever unconsumed bytes the *previous* call on
    this same socket returned -- otherwise, if a response ever arrives
    with extra trailing bytes attached (e.g. a server bug appending stray
    data past its own Content-Length), those bytes would be silently
    dropped instead of being correctly treated as the start of the next
    response.

    Returns (ok, status, error_label, latency, sock_or_None, raw_bytes, leftover_bytes).
    """
    ...
    try:
        ...
        sock.settimeout(READ_TIMEOUT)
        sock.sendall(req)
        raw, new_leftover, truncated = read_one_http_response(
            sock,
            read_timeout=READ_TIMEOUT,
            leftover=leftover,
            request_method=method.encode(),
        )
        ...
        if truncated:
            if not keep_alive:
                sock.close()
                sock = None
            return False, status, "TRUNCATED_RESPONSE", latency, sock, raw, b""
        if status is None:
            if not keep_alive:
                sock.close()
                sock = None
            return False, None, "MALFORMED_RESPONSE", latency, sock, raw, b""
        if not keep_alive:
            sock.close()
            sock = None
            new_leftover = b""
        return True, status, None, latency, sock, raw, new_leftover
        ...
        try:
            ...
            pass
        return False, None, classify_error(e), latency, None, b"", b""
...
def worker_loop(
    ...
    def sample_worthy(err):
        return err in ("MALFORMED_RESPONSE", "TRUNCATED_RESPONSE")
    ...
    while not stop_event.is_set():
        ...
        if cat == "upload":
            ...
            ok, status, err, lat, s, raw, lo = do_single_request(
                "POST",
                UPLOAD_PATH + name,
                {"Content-Type": "text/plain"},
                body,
                keep_alive,
                persistent_sock if keep_alive else None,
                persistent_leftover if keep_alive else b"",
            )
            stats.add(
                "upload", ok, status, err, lat, raw if sample_worthy(err) else None
            )
            if keep_alive:
                persistent_sock, persistent_leftover = s, lo
            # best-effort cleanup, always attempted, own try/except so a
            # failed cleanup never kills the worker
            ok2, status2, err2, lat2, s2, raw2, lo2 = do_single_request(
                "DELETE",
                UPLOAD_PATH + name,
                {},
                b"",
            ...
            stats.add(
                ...
                err2,
                lat2,
                raw2 if sample_worthy(err2) else None,
            )
            if keep_alive:
                persistent_sock, persistent_leftover = s2, lo2
        else:
            ...
            stats.add(cat, ok, status, err, lat, raw if sample_worthy(err) else None)
            if keep_alive:
                persistent_sock, persistent_leftover = s, lo
...
def progress_printer(stats: Stats, stop_event, t_start):
    ...
    while not stop_event.wait(PRINT_INTERVAL):
        ...
        print(
            f"  ... {n} requests so far, {elapsed:.0f}s elapsed, ~{rate:.0f} req/s (last {PRINT_INTERVAL:.0f}s)"
        )
        last_n = n
...
def main():
    ...
    mode = (
        "keep-alive (sequential, non-pipelined)"
        if args.keep_alive
        else "fresh connection per request"
    )
    print(
        f"Stress-testing {HOST}:{PORT} with {args.workers} workers "
        f"[{mode}]"
    ...
    printer = threading.Thread(
        target=progress_printer, args=(stats, stop_event, t_start), daemon=True
    )
