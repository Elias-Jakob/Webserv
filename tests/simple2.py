...
def worker(thread_id):
    ...
    keep_alive = False#(thread_id % 2 == 0)  # Haelfte der Threads keep-alive, Haelfte frisch
    sock = None
    ...
    for _ in range(REQUESTS_PER_THREAD):
        ...
        try:
            ...
            # Egal ob keep-alive oder nicht: nach JEDEM Fehler (Timeout,
            # Reset, ...) ist der Framing-Zustand dieses Sockets nicht mehr
            # vertrauenswuerdig -- z.B. koennten noch ungelesene Reste einer
            # vorherigen Response drinstecken. Niemals weiterverwenden.
            if sock is not None:
                try:
                    sock.close()
                except OSError:
        ...
        with lock:
            if status == SUCCESS_STATUS:
                ...
                fail_count += 1
                print(f"[FAIL] thread={thread_id} keep_alive={keep_alive} "
